			...
			const struct sahara_image *images)
{
	...
	if (ret < 0 || ((size_t)ret != len)) {
		ux_err("failed to write %zu bytes to sahara, sending reset\n", len);
		sahara_send_reset(qdl);
		return;
	}
}
	       ...
	       const char *ramdump_filter)
{
	...
	int n;
	int retry_count = 0;
	const int MAX_SAHARA_RETRIES = 3;
	...
	while (!done) {
		...
		if (n < 0) {
			...
			/* NEW: Try to recover with SAHARA_RESET_CMD */
			if (retry_count < MAX_SAHARA_RETRIES) {
				ux_err("Attempting Sahara reset recovery (attempt %d/%d)...\n", 
					retry_count + 1, MAX_SAHARA_RETRIES);
				
				sahara_send_reset(qdl);
				usleep(500000);  /* Wait 500ms for device to reset */
				
				retry_count++;
				continue;  /* Retry the read */
			}
			
			ux_err("Sahara recovery failed after %d attempts. Device needs power cycle.\n", 
				MAX_SAHARA_RETRIES);
			break;
		}
