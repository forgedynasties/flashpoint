...
if [[ -f "$PREBUILT" ]]; then
    ...
    make -C "$QDL_SRC" -j"$(nproc)" clean all
...
# Helper: remove any existing export of VAR then append the new one
_set_env() {
    local var="$1"
    local val="$2"
    # Strip existing lines for this var (handles both quoted and bare forms)
    sed -i "/^export ${var}[[:space:]]*=/d" "$BASHRC"
    printf 'export %s="%s"\n' "$var" "$val" >> "$BASHRC"
}

# Append a clearly delimited block (idempotent removal handled per-var above)
{
    echo ""
    echo "# ── Factory Flash Station ────────────────────────────────────────"
    echo "# QDL_BIN: path to the qdl binary installed by setup.sh"
    echo "export QDL_BIN=\"$QDL_INSTALL_PATH\""
    echo ""
    echo "# FACTORY_FW_PATH: directory containing the factory-flash firmware"
    echo "# (prog*.elf, rawprogram*.xml, patch*.xml).  SET THIS before running."
    echo "export FACTORY_FW_PATH=\"\""
    echo ""
    echo "# PROD_DEBUG_FW_PATH: directory containing the prod/debug firmware"
    echo "# for the second flash stage.  SET THIS before running."
    echo "export PROD_DEBUG_FW_PATH=\"\""
    echo ""
    echo "# BOOT_TIMEOUT_SEC: seconds to wait for device to boot between stages"
    echo "export BOOT_TIMEOUT_SEC=\"120\""
    echo ""
    echo "# FACTORY_REPORTS_DIR: where flash reports are saved"
    echo "export FACTORY_REPORTS_DIR=\"\$HOME/factory_reports\""
    echo "# ─────────────────────────────────────────────────────────────────"
} >> "$BASHRC"
...
echo "  2. Edit $BASHRC and fill in the two firmware paths:"
